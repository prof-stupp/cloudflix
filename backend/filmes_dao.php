<?php
// filmes_dao.php
// Funções CRUD para o recurso 'filmes' usando PDO com MySQL
...
function createFilme($pdo, $data) {
    ...
    // MySQL não suporta RETURNING, usamos lastInsertId()
    $sql = "INSERT INTO filmes (titulo, genero, cartaz) VALUES (?, ?, ?)";
...
/**
 * Função READ (GET /api.php?resource=filmes ou /api.php?resource=filmes&id=2)
 */
...
function updateFilme($pdo, $id, $data) {
    ...
    try {
        ...
        if ($stmt->rowCount() > 0) {
            ...
            http_response_code(404);
            return array("message" => "Filme não encontrado ou nenhum dado alterado.");
        }
